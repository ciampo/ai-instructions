...
test( 'explicit Copilot repository export refreshes only managed files', async ( t ) => {
	...
	await rm( target );
	const repairOutput = runInstaller( home, arguments_ );
	assert.match( repairOutput, /\[=\].*AGENTS\.md/ );
	assert.match( repairOutput, /\[\+\].*copilot-instructions\.md/ );
	assert.match( repairOutput, /Newly linked\/copied: 1/ );
	assert.match( repairOutput, /Already up to date:  1/ );
	assert.equal( await readFile( target, 'utf8' ), '<!-- Auto-generated by ai-instructions/setup.sh --copilot-concat -->\n<!-- Do not edit manually. Re-run setup.sh to update. -->\n\n@../AGENTS.md\n' );
