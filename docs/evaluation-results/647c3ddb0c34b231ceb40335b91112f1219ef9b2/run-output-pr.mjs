...
const targetRevision = '647c3ddb0c34b231ceb40335b91112f1219ef9b2';
const fixtureRevision = '647c3ddb0c34b231ceb40335b91112f1219ef9b2';
...
function readGitFile( revisionPath ) {
	...
	if ( result.status !== 0 ) {
		throw new Error(
			result.stderr || 'git show ' + revisionPath + ' failed.'
		);
	}
...
async function loadCases( skillNames, installedRoot ) {
	...
	for ( const skill of skillNames ) {
		const fixture = JSON.parse(
			readGitFile(
				fixtureRevision + ':skills/' + skill + '/evals/evals.json'
			)
		);
		for ( const outputCase of fixture.outputCases ?? [] ) {
			const requestedCase = skill + '/' + outputCase.id;
			if (
				options.cases.length &&
				! options.cases.includes( requestedCase )
			) {
				continue;
			}
			const context = readGitFile(
				fixtureRevision +
					':skills/' +
					skill +
					'/' +
...
async function createEvaluationTools( workspace, testCase ) {
	...
	await fs.writeFile(
		...
		[
			'#!/bin/sh',
			'if [ "$1" = "52e35d57534525d5a05421878d8c2d349c37d0c6" ] || [ "$1" = "2222222222222222222222222222222222222222" ]; then',
...
function subjectPrompt( testCase, environment ) {
	...
	const reviewWaitInstruction =
		...
		testCase.caseId === 'current-head-convergence'
			...
			: testCase.skill === 'self-review-pr' &&
				  testCase.caseId ===
						'previous-feedback-and-clear-actions'
				? 'Start the required independent self-review subagent before reading the supplied discussion details. Immediately after the subagent start event, run evaluation-mark-independent-start 2222222222222222222222222222222222222222. This deterministic local command makes that ordering auditable; it is not a remote lookup.'
...
function graderPrompt( evidencePath ) {
	return [
		...
		'Count the assertions in evidence.json before grading. Include exactly one result for every assertion, in order, with consecutive indices from 1 through that count. Do not omit an assertion even when it overlaps another assertion.',
	].join( '\n' );
}
...
function subjectTimeoutForCase( testCase ) {
	return [ 'review-pr', 'review-coordinator', 'self-review-pr' ].includes(
		testCase.skill
	)
	...
	const campaign = {
		...
		runner: {
			...
			subjectTimeoutOverrides: {
				...
				'self-review-pr': extendedSubjectTimeoutMs / 1000,
			},
		},
		schemaVersion: 2,
		summary: undefined,
