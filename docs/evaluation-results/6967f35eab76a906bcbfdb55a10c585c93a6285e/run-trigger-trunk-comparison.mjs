...
async function collectProvenance( skillNames, installedRoot ) {
	...
	for ( const skill of skillNames ) {
		...
		skills.push( {
			...
			match: targetTree === checkoutTree,
		} );
	}

	return {
		method: 'Git archive materialized a read-only comparison-revision skill tree before fixtures were loaded. Fixtures were read from that stage. The checkout skill tree was recorded separately and was not used as the installed comparison source. Every attempt linked exactly the staged inventory into a fresh isolated home.',
