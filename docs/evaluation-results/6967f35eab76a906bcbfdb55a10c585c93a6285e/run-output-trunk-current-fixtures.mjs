...
const targetRevision = '358d2aa41b1b60812c6b16e231ccd44464ba1d92';
const fixtureRevision = '6967f35eab76a906bcbfdb55a10c585c93a6285e';
...
function readRevisionFile( revision, filePath ) {
	...
	if ( result.status !== 0 ) {
		throw new Error(
			result.stderr ||
				'git show ' + revision + ':' + filePath + ' failed.'
		);
	}
...
async function loadCases( skillNames, installedRoot ) {
	...
	for ( const skill of skillNames ) {
		...
		for ( const outputCase of fixture.outputCases ?? [] ) {
			const context = readRevisionFile(
				fixtureRevision,
				'skills/' + skill + '/' + outputCase.context
			);
...
async function collectProvenance( skillNames, installedRoot ) {
	...
	const skills = skillNames.map( ( skill ) => {
		...
		return {
			...
			match: targetTreeObject === checkoutTreeObject,
			name: skill,
	...
	return {
		...
		installedSkillNames,
		method:
			'Git archive materialized the immutable comparison skill tree. Evaluation definitions and contexts were read from the recorded fixture revision. Every case linked the complete staged skill inventory into a fresh isolated home. The checkout skill tree was recorded separately and was not used as the installed comparison source.',
		skills,
		targetTree,
	};
}
