...
Accessibility, public API design, compatibility, performance, security, test quality, internationalization, documentation, and simplicity reviews use direct `review-*` skills. `review-coordinator` can organize the material lanes into one response when the host provides subagents, with a sequential fallback. The repository does not install custom-agent definitions for those capabilities.
