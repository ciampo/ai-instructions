...
The agent layer did not change tools, authority, context isolation, or the resulting analysis enough to justify a second copy of each specialist contract. All three capabilities therefore use direct skills. At the decision revision, the repository did not distribute custom-agent sources; legacy destinations let `install`, `update`, and `remove` clean only repository-owned retired agents, while user-maintained agents remain untouched. The current opt-in coordinator is a separate orchestration role, recorded in [ADR 0006](decisions/0006-review-coordinator.md), and does not restore specialist-agent prompts.
