...
| Exact-head Copilot reviewer requests | Include authority to delegate one request per recorded head without another question; keep connector, CLI, and browser mechanics in the runtime-owned host reviewer-request workflow, which is not distributed as a repository skill | `iterate-pr-review` authority contract and the host workflow that owns the reviewer integration |
