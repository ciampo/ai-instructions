...
| Rebase an authored pull request, then iterate it | The iteration bundle plus one verified rebase and `git push <VERIFIED-PR-HEAD-REMOTE> HEAD:refs/heads/<TASK-BRANCH> --force-with-lease=refs/heads/<TASK-BRANCH>:<RECORDED-REMOTE-HEAD>` | An implicit remote or refspec, a bare lease, plain `--force`, another branch, a history rewrite without the requested rebase, and every excluded iteration action |
...
| Exact-lease history update after an explicitly requested rebase | Include only after patch-replay and remote-head verification; name the verified PR-head remote, push `HEAD:<branch-ref>`, and bind that ref to the recorded head in `--force-with-lease=<ref>:<SHA>` | `repository-maintenance` and `iterate-pr-review` |
