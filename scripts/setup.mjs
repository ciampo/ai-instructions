...
Options:
  ...
  --copilot-concat [DIR]  Export repository Copilot instructions explicitly
...
async function processCopilotExport( directory, state ) {
	...
	const destination = path.join( root, '.github', 'copilot-instructions.md' );
	const expectedContent = copilotProjectExport( await concatInstructions( instructionsDir ) );
	...
	if ( stats && await readFile( destination, 'utf8' ) === expectedContent ) {
		state.upToDate++;
		console.log( `  [=] ${ destination }` );
		return;
	}
	if ( stats && state.options.command === 'install' ) {
		console.warn( `  [warning] ${ destination } is outdated; run update to refresh` );
		state.skipped++;
		return;
	}
