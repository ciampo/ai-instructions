...
import { managedMarkdown } from './lib/formats.mjs';
import {
	isOwnedPath,
	lstatSafe,
	managedFileType,
...
async function processCopilotExport( directory, state ) {
	...
	const expectedContent = managedMarkdown( source );
	const stats = await lstatSafe( destination );
	if ( stats && ! await isOwnedPath( destination, repoDir ) ) {
		console.warn( `  [warning] ${ destination } already exists and was not generated by this script -- skipping` );
		state.skipped++;
		await removeLegacyCopilotExport( root, state );
		return;
	}
	const isCurrent = Boolean( stats ) && await readFile( destination, 'utf8' ) === expectedContent;
	if ( stats && ! isCurrent && state.options.command === 'install' ) {
		console.warn( '  [warning] repository export is outdated; run update to refresh' );
		state.skipped++;
		return;
	}
	if ( state.options.dryRun ) {
		if ( ! isCurrent ) console.log( `  [dry-run] write ${ destination }` );
		await removeLegacyCopilotExport( root, state );
		return;
	}
	if ( isCurrent ) {
...
async function removeLegacyCopilotExport( root, state ) {
	const legacyWrapper = path.join( root, '.github', 'copilot-instructions.md' );
	if ( await managedFileType( legacyWrapper ) !== 'copilot-export' ) {
		return;
	}
	if ( state.options.dryRun ) {
		console.log( `  [dry-run] remove ${ legacyWrapper }` );
		return;
	}
	if ( await removeOwnedPath( legacyWrapper, repoDir ) ) {
		console.log( `  [-] ${ legacyWrapper } (obsolete duplicate Copilot wrapper)` );
		state.stale++;
	}
}
