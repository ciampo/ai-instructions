...
- **[STRONG]** For a GitHub issue or pull request, read its canonical title and enough of its description to understand the requested outcome before setting the task title. Use that metadata for the subject. Do not replace it with a generic repository or activity label such as `Review Gutenberg PR`.
- **[RULE]** Keep the title anchored to the user's primary task. A supporting action, such as opening a follow-up issue during a pull-request review, does not replace the primary reference or subject. Keep the existing title, or combine both subjects only when the user makes both tasks co-equal and the result stays concise.
- For numbered work, use `<phase emoji> [<work type>#<number>] <canonical title>`: `I` for an issue, `A` for a pull request being authored, or `R` for a pull request being reviewed. Use the source title as the default subject, shorten it only for readability, and use the description to clarify an ambiguous source title. Keep the primary reference and subject stable when only the phase changes.
