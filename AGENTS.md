...
- **[RULE]** When the user says `bro`, `say that simply`, asks for plain English, or makes an equivalent request, rewrite the prior explanation without changing its meaning or adding claims. Preserve every conclusion, constraint, caveat, piece of evidence, uncertainty, and exact identifier. For each technical term that remains, add a brief everyday-language definition instead of only repeating the term.
