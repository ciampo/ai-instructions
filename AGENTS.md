...
- **[RULE]** When the user says `bro`, `say that simply`, asks for plain English, or makes an equivalent request, rewrite the prior explanation without changing its meaning or adding claims. Preserve every conclusion, constraint, caveat, piece of evidence, uncertainty, and exact identifier. Briefly explain each necessary technical concept in source-supported everyday language. Do not guess what an identifier does or what state it has.
