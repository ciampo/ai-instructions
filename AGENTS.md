...
- **[STRONG]** For a GitHub issue or pull request, read its canonical title and enough of its description to understand the requested outcome before setting the task title. Use that metadata to write a short subject that accurately describes the work. Prefer a concise paraphrase over copying the source title when it preserves the meaning. Do not replace it with a generic repository or activity label such as `Review Gutenberg PR`.
- **[RULE]** If canonical metadata is unavailable, use user-supplied or otherwise verified task context. If that context is insufficient, keep the current anchored title or wait to set one. Do not guess issue or pull-request details.
