...
- **[RULE]** Keep the complete title anchored to the session's primary task. After the title is accurate, rename it only to correct verified metadata or when the user replaces the primary task. Iteration, fixes, feedback, rebases, CI, verification, commits, pushes, and follow-up artifacts are supporting work. They do not change the purpose emoji, work type, reference, or subject.
