...
5. **Verify**: Run the smallest repository-native checks that directly exercise the accepted fixes before any broader suite. Confirm the repository configuration before retrying a failed tool. In a time-bounded task, complete focused verification and preserve enough time to deliver the replies; run broader checks only when time remains. For a self-contained fixture or incomplete checkout, do not reconstruct missing repository configuration. Use the available focused checks and report the exact remaining gap. Push only when explicitly requested.
6. **Verify fixes against actual code**: Refresh the PR metadata and remote refs without changing the local branch. If either SHA changed, rebuild the snapshot and re-evaluate feedback before reporting. For a supplied self-contained snapshot, re-read its recorded boundaries, feedback, and resulting local files instead of making a live refresh. Then check the actual code to confirm the fix is correct -- do not just trust that a commit exists. Report which issues are properly fixed and which still need work.
7. **Prepare reply document**:
   ...
   - For chat delivery, name the focused verification commands and results before the replies. If an optional broad check did not run or finish, report that gap without withholding the replies.
