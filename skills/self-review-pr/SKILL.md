...
1. Gather a fresh PR snapshot: use field-limited `gh pr view` to capture the base and head SHAs plus the PR title and description, and capture the base-to-head commit log. Use `gh pr diff --name-only` plus explicit local refs at both captured SHAs to inspect source, and `gh pr checks` plus failed Actions logs for CI. Use the connector when merged discussion or resolved review-thread state is material, or when `gh` authentication or capability cannot gather the needed state. Re-read PR metadata before concluding and refresh the snapshot if either captured SHA changed.
