...
2. **Establish scope.** Use `gh pr diff --name-only` to find changed files. Fetch the captured head into an explicit local ref, verify that ref resolves to `headRefOid`, and use local Git to read the relevant file diffs and full source. Do not inspect a mutable branch name, and do not rely on `FETCH_HEAD`, which another fetch can replace.
...
5. **Confirm freshness.** Re-read field-limited PR metadata immediately before forming conclusions. If `headRefOid` changed, the snapshot is stale: repeat these steps and review only the new snapshot.
