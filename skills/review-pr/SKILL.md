...
7. Use the `draft-review-comment` skill to write the full review to `<pr-number>-review.md` in the OS temporary directory. Open it when supported; otherwise return the path. Print nothing else in chat beyond a one-line confirmation unless the user explicitly requested chat snippets.
