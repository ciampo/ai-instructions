...
2. Follow the snapshot procedure. Review only the pinned PR diff and its changed source; do not substitute the current local branch or a later remote state. Record changed-file and added/deleted-line counts from that diff when available. Distinguish substantive source and test changes from generated, lockfile, formatting, and mechanical churn without excluding any of them from review. Pass this scope summary with any coordinator handoff; do not infer low risk from a small diff.
