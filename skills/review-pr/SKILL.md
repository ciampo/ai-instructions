...
7. If `review-coordinator` was not selected, load a specialist skill directly only when its domain is materially in scope:
   ...
   - Use `review-simplicity` when the user explicitly requests a simpler or deletion-first approach, or when the change adds material state, indirection, dependencies, duplication, or speculative abstraction that may be avoidable. Do not use it for generic stylistic preferences.
