...
The change adds `POST /accounts/:accountId/delete`, which verifies only that a session exists and does not compare the session user to `accountId`. It also changes a persisted preference from `sidebarVisible` to `showSidebar` without migration. Account identifiers are discoverable by signed-in users, and existing installations can still have the previous persisted key. No dependency, browser, or deployment evidence is supplied.
