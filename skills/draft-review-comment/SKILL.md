...
- For a full review document or full review in chat, read [the review-report format](references/review-report.md). Keep the overall assessment, previous-feedback status, and actions separate. A direct request for individual comments or replies needs only those drafts, not a full report or a new PR investigation. Use supplied feedback assessments; mark missing evidence as unchecked rather than claiming all feedback is addressed.
...
- For chat delivery, do not create or open a local file. Return the requested full report or only the requested comments or replies.

## Steps

1. Identify the pull request, code, and concern that each comment addresses.
2. Draft the smallest useful response in a friendly, natural tone. Lead with what fails or what should change, using words a teammate could say aloud. Explain a necessary technical term briefly when the reader needs it. Keep exact identifiers and uncertainty, but avoid packing several mechanisms into a short sentence. The GitHub thread already supplies context, so do not repeat the original feedback, code location, investigation, or proof in the visible comment unless the reader needs it to understand or act.
   ...
   - Check simplicity separately from length: can the reader identify the problem and next step on the first read? Rewrite dense clauses instead of merely cutting words or joining them with semicolons. Short technical shorthand is still dense.
...
3. Follow **Output Delivery**. For comments-only requests, use one copy-pasteable section per comment thread or review point. For a full report, keep actions separate from suggested text as the report reference describes.
...
5. If I edit or refine the draft, preserve its delivery mode: update and re-open the existing review document for file delivery, or return the revised report or requested comments for chat delivery.
