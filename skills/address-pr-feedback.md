...
5. **Prepare reply document**: For each addressed comment, draft a short reply explaining what was done. For won't-fix items, draft a respectful explanation of why. Write all replies to a single markdown document following the `draft-review-comment` skill — each reply section references the exact file path and line range of the original comment, is self-contained, and is copy-pasteable. The document is written to `.ai-reviews/<pr-number>-replies.md` and opened in the editor (per `interaction-preferences.md`: never post to GitHub directly).
