...
5. **Prepare reply snippets**: For each addressed comment, draft a short reply explaining what was done. For won't-fix items, draft a respectful explanation of why. Output all replies as separate markdown snippets in the chat (per `interaction-preferences.md`: never post to GitHub directly).
