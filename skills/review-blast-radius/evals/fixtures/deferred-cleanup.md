# Deferred session-cleanup fixture
...
account after `closeSession` returns. The only supplied executable evidence is
[`deferred-cleanup-probe.mjs`](deferred-cleanup-probe.mjs). Run it with:

```sh
node skills/review-blast-radius/evals/fixtures/deferred-cleanup-probe.mjs
```

No other consumers, integration tests, or running application are available.
