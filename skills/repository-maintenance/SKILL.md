...
- Read [writing conventions](references/writing-conventions.md) when writing commits, changelogs, pull-request descriptions, public API documentation, or error messages, or when repairing an explanation or artifact after a plain-language request.
...
- Treat the complete title as the stable identity of the session's primary task, not its current workflow step. After the title is accurate, rename it only to correct verified metadata, when the user replaces the primary task, when the user explicitly makes another task co-equal, or when an issue task becomes the authored pull request that is now the session's primary delivery. Iteration, fixes, feedback, rebases, CI, verification, commits, pushes, and follow-up artifacts do not make another task co-equal or change the title.
- When several primary tasks are co-equal, omit the bracketed reference and use a concise batch subject instead of listing every number.
