...
- Treat the complete title as the stable identity of the session's primary task, not its current workflow step. After the title is accurate, do not change its emoji, work type, reference, or subject for iteration, fixes, feedback, rebases, CI, verification, commits, pushes, or follow-up artifacts. Rename it only to correct verified metadata or when the user replaces the primary task.
