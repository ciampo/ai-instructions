...
2. Capture a fresh PR snapshot: canonical URL, title, base and head revisions, changed files, existing review state, and CI status. Accept a supplied immutable context that records this boundary and its review evidence without remote lookup. Otherwise, verify one exact local checkout or authenticated supported pull-request read interface. If no route can provide the boundary and diff, stop after that capability check and request the missing snapshot; do not enumerate unrelated local files, try unauthenticated web clients, or retry alternate network routes. Record the head revision for this round. Rebuild the snapshot whenever it changes.
