...
- When the invoking request explicitly includes a rebase, extend the bundle only to the resulting update through `git push <VERIFIED-PR-HEAD-REMOTE> HEAD:refs/heads/<TASK-BRANCH> --force-with-lease=refs/heads/<TASK-BRANCH>:<RECORDED-REMOTE-HEAD>` after patch-replay and remote-head verification. Do not use an implicit remote or refspec, a bare lease, plain `--force`, rewrite another branch, or infer history-rewrite authority when the request did not include a rebase.
...
1. When a rebase is explicitly requested, refresh the intended base and remote pull-request head, rebase the task branch, and verify patch replay. Run required project checks and any required model-backed evaluations for the exact rebased commit before publishing it. If verification or a required evaluation fails, do not push. Otherwise, publish only with the verified remote, explicit task-branch refspec, and `<branch-ref>:<recorded-head>` lease above, then confirm that the local, remote, and pull-request heads match before reviewing. Do not use this step to rewrite history when the request did not include a rebase.
