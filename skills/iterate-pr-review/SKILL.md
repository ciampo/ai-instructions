...
- Treat an explicit request to run this workflow as one task-scoped authority bundle for the identified pull request: request one fresh Copilot review for each exact head when needed; run the independent review; apply accepted fixes; run required verification; make coherent commits; push the same pull-request branch; and repeat within the configured round limit. The bundle also permits updating an existing authored draft pull request only when the workflow must record its exact-head evaluation result. Respect narrower limits such as review-only, local-only, do-not-commit, or do-not-push instructions.
...
- Before the first affected action, record the repository, pull request, branch, review destination, payload class, and round limit. If a required action is outside the bundle, ask at most one consolidated authority question and retain the answer while those values remain unchanged. A different repository, pull request, branch, destination, payload class, or exceeded round limit requires a new authority decision.
...
- Keep the agent's own public comments, replies, or reviews, review-thread resolution, unrelated pull-request metadata, ready-for-review transitions, merges, and releases outside the bundle. A Copilot reviewer request for the recorded head is part of the review loop; it does not authorize those other writes.
...
6. Implement accepted fixes within the active bundle. Verify the changed behavior with the relevant project checks. Run required model-backed evaluations for the exact head only within the recorded payload and destination boundary. Make a coherent commit, push the same pull-request branch, refresh the pull request, and verify that its remote head matches the new commit before starting the next round. If the request explicitly limited the workflow to local or uncommitted work, report the verified local state and stop because it cannot begin a new remote-review round.
