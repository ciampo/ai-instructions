...
- When the invoking request explicitly includes a rebase, extend the bundle only to the resulting update of the same pull-request branch with `--force-with-lease=refs/heads/<TASK-BRANCH>:<RECORDED-REMOTE-HEAD>` after patch-replay and remote-head verification. Do not use a bare lease, plain `--force`, rewrite another branch, or infer history-rewrite authority when the request did not include a rebase.
